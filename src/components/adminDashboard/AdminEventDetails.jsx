...
function AdminEventDetails() {
  ...
  const handleAcceptReject = async (teamId, action) => {
    const confirmText = `Are you sure you want to ${action === "accept" ? "accept" : "reject"} this team?`;
    if (!window.confirm(confirmText)) return;

    try {
      const endpoint = `${BASE_URL}/participants/${action}/${teamId}`;
      const response = await axios.get(endpoint);
      if (response.status !== 200) throw new Error("Server responded with an error status");
      ...
      const message = action === "accept" ? "Team accepted successfully!" : "Team rejected successfully!";
      showNotification(message, "success");
    } catch (err) {
      showNotification(`Failed to ${action} the team. Please try again.`, "error");
  ...
  const downloadAsExcel = () => {
    ...
    try {
      ...
      XLSX.writeFile(wb, `${eventData.event.title}_Participants_${new Date().toLocaleDateString()}.xlsx`);
  ...
  const getStats = () => {
    ...
    const totalMembers = eventData.participants.reduce(
      (acc, team) => acc + team.teamMembers.length + 1, // +1 for team leader
      0
    );
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-12">
      {/* Notification */}
      <div id="notification" className="fixed top-4 right-4 p-4 rounded-lg shadow-lg bg-green-500 text-white opacity-0 transition-opacity duration-500"></div>
        ...
        <div className="container mx-auto">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <div>
              ...
              <p className="text-blue-100 mt-1">{eventData?.event?.date || "Date not available"}</p>
              <p className="text-blue-100 mt-1">{eventData?.event?.location || ""}</p>
            </div>
      ...
      <div className="container mx-auto px-4 -mt-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 mb-8">
          <StatCard title="Total Teams" value={stats.totalTeams} icon="users" color="blue" />
          <StatCard title="Accepted" value={stats.acceptedTeams} icon="check" color="green" />
          <StatCard title="Pending" value={stats.pendingTeams} icon="clock" color="yellow" />
          <StatCard title="Total Participants" value={stats.totalMembers} icon="user" color="indigo" />
          <StatCard title="Unique Colleges" value={stats.uniqueColleges} icon="academic-cap" color="purple" />
          <StatCard title="Latest Registration" value={stats.latestRegistration} icon="calendar" color="pink" isText />
        </div>

        {/* Filters */}
        <div className="bg-white rounded-lg shadow-md p-4 mb-6">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <h2 className="text-xl font-bold text-gray-800">Team Registrations</h2>
        ...
        <div className="bg-white shadow-md rounded-lg overflow-hidden">
          {displayedParticipants.length === 0 ? (
            <div className="p-8 text-center">
              ...
              <h3 className="text-lg font-medium text-gray-700 mb-1">No teams found</h3>
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-gray-200">
                <thead className="bg-gray-50">
                  <tr>
                    <th className="px-6 py-3 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Team</th>
                    <th className="px-6 py-3 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Leader</th>
                  ...
                  {displayedParticipants.map((team) => (
                    <React.Fragment key={team._id}>
                      <tr className="hover:bg-gray-50 transition duration-150">
                        ...
                        <td className="px-6 py-4">
                          <div className="font-medium text-gray-800">{team.teamLeader.name}</div>
                              ...
                              {team.teamMembers.length === 0 ? (
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                                  {team.teamMembers.map((member) => (
                                    <div key={member._id} className="p-4 bg-white border rounded-lg shadow-sm hover:shadow transition duration-150">
...
function StatCard({ title, value, icon, color, isText = false }) {
  ...
  return (
    <div className="bg-white rounded-lg shadow-md p-6 flex flex-col items-center hover:shadow-lg transition duration-300">
      ...
      <p className={`text-2xl font-bold ${isText ? "text-gray-700 text-lg" : `text-${color}-600`}`}>
        {value}
      </p>
    </div>
  );
